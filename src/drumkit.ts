...
export class DrumKit {
    ...
    private baseDrum = new Tone.MembraneSynth({
        volume: -6
    }).toMaster();
    private lowTom = new Tone
        .MembraneSynth({
            pitchDecay: 0.008,
            envelope: { attack: 0.01, decay: 0.5, sustain: 0 },
        })
        .toMaster();
    private midTom = new Tone
        .MembraneSynth({
            pitchDecay: 0.008,
            envelope: { attack: 0.01, decay: 0.5, sustain: 0 },
        })
        .toMaster();
    private HighTom = new Tone
        .MembraneSynth({
            pitchDecay: 0.008,
            envelope: { attack: 0.01, decay: 0.5, sustain: 0 },
        })
    ...
    private pitchPlayers = [
        (time: number, velocity = 1) =>
            this.baseDrum.triggerAttackRelease('C2', '8n', time, velocity),
        (time: number, velocity = 1) =>
            this.snareDrum.triggerAttackRelease('16n', time, velocity),
        (time: number, velocity = 1) =>
            this.closedHihat.triggerAttack(time, 0.3, velocity),
        (time: number, velocity = 1) =>
            this.openHihat.triggerAttack(time, 0.3, velocity),
        (time: number, velocity = 0.5) =>
            this.lowTom.triggerAttack('G3', time, velocity),
        (time: number, velocity = 0.5) =>
            this.midTom.triggerAttack('C4', time, velocity),
        (time: number, velocity = 0.5) =>
            this.HighTom.triggerAttack('F4', time, velocity),
        (time: number, velocity = 1) =>
            this.crashCymbal.triggerAttack(time, 1.0, velocity),
        (time: number, velocity = 1) =>
            this.rideCymbal.triggerAttack(time, 0.5, velocity)
    ];
